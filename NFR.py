...
def NFR(n, A, b, lamda,psi, 
    ...
    while norm_gk >= Tolerance * (1 + abs(fk)) and NI < MaxIter:
        ...
        # NFR direction update
        rho2 =0.8
        psi2=0.4
        num = norm_gk1**2+rho2*(norm_gk1*np.abs(np.dot(gk1,dk))*np.abs(np.dot(gk1,gk)))/(np.dot(dk,gk)*norm_gk)
        denum =norm_gk**2 + psi2*norm_gk1*np.linalg.norm(dk)
        betak =num/denum
        dk = -gk1 + betak*dk
