...
def NTDCG2(n, A, b, lamda,psi, 
    ...
    while norm_gk >= Tolerance * (1 + abs(fk)) and NI < MaxIter:
        ...
        betak = np.dot(gk1, yk) / denum  # NMHS
        thetak = np.dot(gk1, dk) / denum  # NM-Thetak

        # b1=np.min(1/EPSL, np.max(b1, EPSL))
        # Update search direction
        dk = -b1*gk1 + betak*dk - thetak*yk #  NEW SEARCH DIRECTION
        # END OF DIRECTION!
