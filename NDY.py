...
def NDY(n, A, b, lamda,psi, 
    ...
    while norm_gk >= Tolerance * (1 + abs(fk)) and NI < MaxIter:
        ...
        yk = gk1 - gk
        # NDY direction update
        rho1 =0.7
        psi1=1.5
        num = norm_gk1**2+rho1*(norm_gk1*np.abs(np.dot(gk1,dk))*np.abs(np.dot(gk1,gk)))/(np.dot(dk,gk)*norm_gk)
        denum =np.dot(dk,yk) + psi1*norm_gk1*np.linalg.norm(dk)
        betak =num/denum
        dk = -gk1 + betak*dk
