...
def SCG2(n, A, b, lamda,psi, 
    ...
    while norm_gk >= Tolerance * (1 + abs(fk)) and NI < MaxIter:
        ...
        # SCG2 direction update
        yk = gk1 - gk
        rho3=0.01
        normd=np.linalg.norm(dk)
        thetak3=rho3+np.linalg.norm(yk)/normd+1
        num = np.minimum(np.abs(np.dot(gk1,yk)), norm_gk1**2)        
        betak =num/normd**2

        dk=-thetak3*gk1+betak*dk
